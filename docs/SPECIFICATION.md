...
| `OracleUpdated`           | (NOT EMITTED — see note)      | (was: pool, oracle_signer, new_fair_value, new_spread_bps, new_nonce, new_ttl, slot)                                                         |
...
>
> **`OracleUpdated` not emitted (v0.6+)**: every field it would carry
> (`fair_value`, `spread_bps`, `oracle_nonce`, `current_mode_ttl`,
> `last_oracle_update_slot`) is written to `PoolState` in the same ix.
> Subscribers should use `Connection.onAccountChange(pool_state, decodePoolState, ...)`
> instead — same data, ~600–800 CU/push saved on the keeper hot path. The
> SDK retains the `OracleUpdatedData` type + decoder for legacy log replay
> only.
>
> Cost: ~100–600 CU per emit (was ~600–1500 CU pre-v0.6; sol_log_data syscall
> + compact bodies). Fits comfortably inside every instruction's CU budget.
...
| Version | Date       | Change |
|---|---|---|
| v0.6    | 2026-05-24 | **`update_oracle` hot-path 5086 → 313 CU (-94%)** matching HumidiFi-class venues. (1) Skip `Instruction: UpdateOracle` log line in dispatcher (lib.rs). (2) Drop `OracleUpdated` event emission entirely — every field is in `PoolState`, subscribers migrate to `onAccountChange`. (3) Stack-allocated emit buffer (no `Vec` heap alloc); benefits every other event too. (4) Per-event bytewise pack functions replace Borsh trait-dispatch (`borsh::BorshSerialize::serialize` still derived for the size-parity test, but never invoked at runtime). ExecuteSwap 20965→18289 (-13%), ProposeAdmin 9362→6201 (-34%), SetPaused 4113→2551 (-38%), all rotates ~5950→~2850 (-52%). (5) `OracleUpdated` event struct compacted (drop `oracle_signer` + `slot`, both derivable) — kept for legacy log replay only. (6) `update_oracle` drops the discriminator check on `pool_state` — the `ACCOUNT_SIZE` gate already disambiguates (PoolState=331B vs QuoteNonceMarker=64B vs AdminRotationProposal=120B). Other ixs keep the discriminator check. (7) `update_oracle` collapses 4 contiguous field writes (`fair_value`..`skew_params`) into one `copy_from_slice` from `ix_data[0..46]` (matches state[224..270] verbatim). (8) `measure-cu.sh` attributes unlabeled consumed-CU samples to UpdateOracle. (9) `events.rs` adds a `size_tests` parity test: every bytewise packer's output length must equal `borsh::BorshSerialize::serialize` for the same struct, so a field add/remove that desyncs SDK decoders fails compile-time. (10) Doc-only: SPEC §3.13 marks OracleUpdated NOT EMITTED; CORE/INCIDENT subscriber pattern updated. |
