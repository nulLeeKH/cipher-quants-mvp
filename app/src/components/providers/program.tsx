...
interface ProgramCtx {
  /** Read-only Anchor Program (always available; uses connection only). */
  readonlyProgram: Program<Protocol>;
  /** Signing Anchor Program (only when wallet connected). */
  signingProgram: Program<Protocol> | null;
  /** Configured program id (env override or IDL default). */
  programId: PublicKey;
}
...
export function ProgramProvider({ children }: { children: React.ReactNode }) {
  ...
  const readonlyProgram = React.useMemo<Program<Protocol>>(() => {
    ...
    return new Program<Protocol>(IDL as unknown as Protocol, provider);
  }, [connection]);
